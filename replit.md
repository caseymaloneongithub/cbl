...
- **Premium Section**: Gated behind `hasPremiumAccess` or super admin status. Nav uses a green dropdown with two pages:
  - **Player Stats** (`/premium/stats`): Advanced player statistics (WAR, xBA, xOBP, xSLG splits for hitters; WAR, xERA, xK/9, xBB/9, xWHIP splits for pitchers), CSV upload by season, sortable columns, CBL team filter. Also shows PA for hitters and GS/IP for pitchers from `mlb_player_stats`.
  - **WAR Rankings** (`/premium/war-rankings`): League teams ranked by total, hitter, and pitcher WAR with bar chart visualization. Includes toggle to exclude negative WAR players from totals.
