...
export async function sendFreeAgentClaimEmail(
  to: string,
  leagueName: string,
  ...
  const posInfo = [playerPosition, playerMlbTeam].filter(Boolean).join(', ');
  const rosterLabel = rosterType.toUpperCase();
  const txLink = `${appUrl}/transactions`;

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="font-family: 'Roboto', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #e65100 0%, #f57c00 100%); padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
    <h1 style="color: white; margin: 0; font-size: 20px;">${leagueName} — Free Agent Claim</h1>
  </div>
  <div style="background: #f9f9f9; padding: 25px; border-radius: 0 0 8px 8px;">
    <p style="font-size: 15px; margin: 0 0 15px;"><strong>${claimingTeamName}</strong> has claimed a free agent:</p>
    <div style="background: white; border: 1px solid #ddd; border-radius: 6px; padding: 15px; text-align: center;">
      <p style="margin: 0; font-size: 18px; font-weight: bold;">${playerName}</p>
      ${posInfo ? `<p style="margin: 4px 0 0; font-size: 13px; color: #555;">${posInfo}</p>` : ''}
      <span style="display: inline-block; margin-top: 8px; background: ${rosterType === 'mlb' ? '#1565c0' : '#6a1b9a'}; color: white; font-size: 11px; padding: 2px 8px; border-radius: 3px;">${rosterLabel}</span>
    </div>
    <div style="text-align: center; margin-top: 20px;">
      <a href="${txLink}" style="background: linear-gradient(135deg, #e65100 0%, #f57c00 100%); color: white; padding: 12px 28px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px; display: inline-block;">View Transactions</a>
    </div>
    <hr style="border: none; border-top: 1px solid #ddd; margin: 20px 0;">
    <p style="color: #999; font-size: 11px; text-align: center;">This notification was sent by ${APP_NAME}.</p>
  </div>
  ...
  const text = `${leagueName} — Free Agent Claim\n\n${claimingTeamName} has claimed ${playerName}${posInfo ? ` (${posInfo})` : ''} [${rosterLabel}].\n\nView transactions: ${txLink}\n`;

  return sendEmail({
    to,
    subject: `${leagueName} — FA Claim: ${playerName} → ${claimingTeamName}`,
    html,
    text,
  });
}
