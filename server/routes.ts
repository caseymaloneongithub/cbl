...
export async function registerRoutes(
  ...
  app.post("/api/leagues/:id/roster-assignments/claim", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (todaysClaims.length > 0) {
        return res.status(429).json({ message: `You have already claimed a free agent for your ${rosterType.toUpperCase()} roster today. You can claim again after midnight ET.` });
      }
  ...
  app.get("/api/drafts/:id/timing", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const deadlinePassedSlots = unmadeSlots.filter(slot => {
        ...
        const priorResolved = sortedSlots
          .filter(s => s.overallPickNumber < slot.overallPickNumber)
          .every(s => isSlotResolved(s, strictMode));
        return priorResolved;
      });

      let currentSlot: typeof sortedSlots[0] | null = null;
      for (const slot of unmadeSlots) {
        if (slot.skippedAt) continue;
        if (slot.round > 1 && slot.deadlineAt && new Date(slot.deadlineAt).getTime() <= now.getTime()) continue;

        if (slot.overallPickNumber === 1) {
          if (new Date(slot.scheduledAt).getTime() <= now.getTime()) {
            currentSlot = slot;
          }
          break;
        }
  ...
  app.post("/api/drafts/:id/start-picks-now", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (allSlots.some(s => !!s.madeAt || !!s.skippedAt)) {
        return res.status(400).json({ message: "Picks have already been made in this draft" });
      }
...
async function sendPickNotificationEmails(
  ...
  try {
    console.log(`[DraftPickEmail] Starting for draft ${draftId}, slot ${madeSlotId}${options?.isSkipped ? " (skipped)" : ""}`);

    const draft = await storage.getDraft(draftId);
    if (!draft) { console.log(`[DraftPickEmail] Draft ${draftId} not found, skipping`); return; }
    ...
    if (!madeSlot) { console.log(`[DraftPickEmail] Slot ${madeSlotId} not found in ${allSlots.length} slots, skipping`); return; }
    ...
    if (recipients.length === 0) { console.log(`[DraftPickEmail] No recipients for draft ${draftId} pick #${madeSlot.overallPickNumber}${isDev ? " (dev mode)" : ""}`); return; }
